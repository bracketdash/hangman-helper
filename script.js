...
for(var x=4;x<9;x++){
	var str = dict[x].replace(/([a-z])(?![A-Z])/g,"\"$1\":{").replace(/([a-z])(?![^A-Z])/g,"\"$1\":");
	str = '{' + str.replace(/([A-Z]+)/g,"\"$1\"").toLowerCase().replace(/""([a-z])":"/g,"\",\"$1\":\"");
	for(var y=2;y<8;y++){
		if(x > y){
...
function rootExists(w,t){
	var test = dict[t][w[0]], len = w.length;
	for(var i=2;i<8;i++){
		if(len > i){
			if(test[w[i-1]]){
				test = test[w[i-1]];
...
function builder(options, callback){
	...
	var lv = {a:1,b:4,c:4,d:2,e:1,f:4,g:3,h:3,i:1,j:10,k:5,l:2,m:4,n:2,o:1,p:4,q:10,r:1,s:1,t:1,u:2,v:5,w:4,x:8,y:3,z:10},
		bp = options.bp, bt = options.bt, wl = {}, wrds = [], ph = {}, pht = {1:options.t}, root = {};
...
function solver(){
	...
	// set up our vars
	var cnts = {a:0,b:0,c:0,d:0,e:0,f:0,g:0,h:0,i:0,j:0,k:0,l:0,m:0,n:0,o:0,p:0,q:0,r:0,s:0,t:0,u:0,v:0,w:0,x:0,y:0,z:0},
		abc = 'abcdefghijklmnopqrstuvwxyz', slen = $('#solver-slots input:visible').length, slot = [], perc = [],
		xyz = abc, wrds = 0, tkr = {}, lmt = {}, bot = {};
	...
	if(wrds > 0){
		...
		$.each(cnts,function(letr,cont){
			$('#l-'+letr).find('.prob').html((Math.round((cont/wrds)*1000)/10)+'%');
		...
		$('#solver .status').html('No words found. Double check strikes and known slots.');
	}
}
...
$(function(){
	...
	$('.bonus-position').click(function(){
		...
		if($(this).attr('id') === 'bp-x'){
			...
			$('#tray').val('');
			$('#builder .status').html('Press enter once you have entered your tray to view possible words.');
			...
			switch (pos) {
				...
				case '2':btl = 44;break;
				case '3':btl = 86;break;
				case '4':btl = 128;break;
				case '5':btl = 170;break;
				case '6':btl = 212;break;
				case '7':btl = 254;break;
				case '8':btl = 296;
			}
			$('#bonus-type-container').show().css('left',btl+'px');
	...
	$('#builder form').submit(function(e){
		e.preventDefault();
		if($('#tray').val().length == 12 && $('.bonus-position.selected').length == 1 && $('.bonus-position.selected').text().length == 2){
			builder({
				...
				if(wlen){
					var res = '';
					for(x=0;x<wlen;x++){
						res += '<div class="word-score"><div class="score">'+wrds[x][1]+'</div><div class="word">'+wrds[x][0]+'</div></div>';
					}
					$('#builder .results').html(res);
					$('#builder .status').html('Found <b>'+wlen+'</b> possible words.');
				} else {
					$('#builder .status').html('No words could be formed. Double check your tray.');
				}
			});
		} else {
			$('#builder .status').html('Please select a bonus and enter all 12 letters of the tray.');
		}
	});
	...
	$('.word-length').click(function(){
		...
		$('#solver .status').html('Probabilities will update automatically. Click on a letter to mark it as a strike.');
		$('.letter-prob').removeClass('strike').removeClass('suggested').removeClass('known').find('.prob').html('0%');
		$(this).siblings('.selected').removeClass('selected').end().addClass('selected');
		switch($(this).attr('id').substring(3)){
			case "4":$('#slot5,#slot6,#slot7,#slot8').hide();break;
			case "5":$('#slot5').show();$('#slot6,#slot7,#slot8').hide();break;
			case "6":$('#slot5,#slot6').show();$('#slot7,#slot8').hide();break;
			case "7":$('#slot5,#slot6,#slot7').show();$('#slot8').hide();break;
			case "8":$('#slot5,#slot6,#slot7,#slot8').show();break;
		}
		$('#slot1').select();
	});
	...
	$('#wl-x').click(function() {
		$('#wl-8').addClass('selected').siblings('.selected').removeClass('selected');
	...
	$('#solver input').keyup(function(e) {
		if(e.keyCode === 37){
			...
			$(this).next().select();
		} else if((e.keyCode >= 65 && event.keyCode <= 90) || event.keyCode === 8 || event.keyCode === 46 || event.keyCode === 16){
